...
export function validateSignup() {
  ...
  function handleChange(e) {
    ...
    passwordConfirmValidation(name);
  }

  /**
   * 비밀번호 확인 필드 검증 함수
   * - 비밀번호 필드가 변경되었을 때 비밀번호 확인 필드 검증
   */
  function passwordConfirmValidation(name) {
    const isPasswordField =
      name === "password" || name === "passwordConfirm";
    const bothTouched = touched.password && touched.passwordConfirm;
  
    if (!isPasswordField || !bothTouched) return;
  ...
  function updateSubmitState() {
    const allValidated = Object.keys(signupSchema.shape).every(
      (key) => validated[key]
    );
